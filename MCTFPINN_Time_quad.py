...
import os
import pandas as pd
from scipy.special import roots_genlaguerre, gamma, roots_jacobi
...
quad_w = quad_w / (args.lambda_x ** (2 - args.alpha))
...
class PINN:
    def __init__(self):
        ...
        self.norm = (args.lambda_x ** (2 - args.alpha)) / gamma(2 - args.alpha)
    ...
    def resample(self, rng): # sample random points at the begining of each iteration
        ...
        return xf, tf, xi, t0, xi_test, r_test, t0_test, tau_test, keys[9]
    ...
    def get_loss_pinn(self, params, xf, tf, xi, t0, ff):
        f = self.r_lap_pred_fn(params, xf, tf, xi, self.quad_x)
        f = f * self.quad_w.reshape(-1, 1) * self.norm
        f = f.sum(0)

        ft1 = self.r_t1_pred_fn(params, xf, tf, self.quad_t)
        ft1 = ft1 * self.quad_wt.reshape(-1, 1)
        ft1 = ft1.sum(0)
    ...
    def step_pinn(self, params, opt_state, rng):
        xf, tf, xi, t0, xi_test, r_test, t0_test, tau_test, rng = self.resample(rng)
        ...
        current_loss, gradients = jax.value_and_grad(self.get_loss_pinn)(params, xf, tf, xi, t0, ff)

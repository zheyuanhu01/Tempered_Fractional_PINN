...
parser.add_argument('--lambda_x', type=float, default=1.0) # tempered fractional lambda
parser.add_argument('--lambda_x_L', type=float, default=1.0) #
parser.add_argument('--lambda_x_H', type=float, default=1.0) # 

parser.add_argument('--alpha', type=float, default=0.55) # tempered fractional alpha
parser.add_argument('--alpha_L', type=float, default=0.3) #
parser.add_argument('--alpha_H', type=float, default=0.7) # 
...
quad_x, quad_w = roots_genlaguerre(args.N_mc, 1 - args.alpha_H)
quad_x = quad_x / args.lambda_x
quad_w = quad_w / (args.lambda_x ** (2 - args.alpha_H))
...
class PINN:
    ...
    def resample(self, rng): # sample random points at the begining of each iteration
        ...
        r = jax.random.gamma(keys[4], 2 - args.alpha_H, shape=(N_mc,)) # / args.lambda_x
    ...
    def residual_lap(self, params, x, xi, r, alpha, lam):
        ...
        const = 0.5 * (lam ** alpha)
        return const * (2 * u - u_plus - u_minus) / r ** 2 * r ** (args.alpha_H - alpha)
    
    def get_loss_pinn(self, params_all, xf, uf, xi, r, ff):
        alpha, params = params_all['alpha'], params_all['pinn']
        alpha = jax.nn.sigmoid(alpha) * (args.alpha_H - args.alpha_L) + args.alpha_L
        lam = args.lambda_x # jax.nn.sigmoid(lam) * 0.2 + 1
        
        data_loss = jnp.mean((self.u_pred_fn(params, xf) - uf) ** 2)

        f = self.r_lap_pred_fn(params, xf, xi, self.quad_x, alpha, lam)
        f = f * self.quad_w.reshape(-1, 1)
        f = f.sum(0)
    ...
    def train_adam(self):
        ...
        for n in tqdm(range(self.epoch)):
            ...
            alpha = jax.nn.sigmoid(self.params['alpha']) * (args.alpha_H - args.alpha_L) + args.alpha_L
            if n%1000==0: print('epoch %d, data loss: %e, fpinn loss: %e, alpha: %e'%(n, current_loss[1][1], current_loss[1][0], alpha))

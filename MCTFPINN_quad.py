...
class PINN:
    ...
    def resample(self, rng): # sample random points at the begining of each iteration
        ...
        N_mc = args.N_mc_test
        xi_test = jax.random.normal(keys[3], shape=(N_mc, args.dim))
        xi_test = xi_test / jnp.linalg.norm(xi_test, axis=1, keepdims=True)

        r_test = jax.random.gamma(keys[4], 2 - args.alpha, shape=(N_mc,)) / args.Lambda
        r_test = jnp.clip(r_test, a_min=args.epsilon)

        return xf, xi, xi_test, r_test, keys[5]
    ...
    def get_loss_pinn(self, params, xf, xi, ff):
        f = self.r_pred_fn(params, xf, xi, self.quad_x) # N_quad, N_mc, N_f
    ...
    def step_pinn(self, params, opt_state, rng):
        xf, xi, xi_test, r_test, rng = self.resample(rng)
